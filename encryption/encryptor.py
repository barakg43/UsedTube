...
class Encryptor:
    ...
    def get_cover_video_metadata(self, cover_video):
        self.strategy.dims = (int(cover_video.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cover_video.get(cv2.CAP_PROP_FRAME_HEIGHT)))
    ...
    def encrypt(self, file_to_encrypt: BufferedReader, cover_video_path: str, out_vid_name):
        """"
        :parameter file_to_encrypt an open file descriptor in 'rb' to the file 
        :parameter video_capture_to_cover a cv2 VideoCapture to intertwine the encrypted file with
        :returns output path 
        """
        cover_video = cv2.VideoCapture(cover_video_path)
        self.collect_metadata(file_to_encrypt, cover_video)
        output_video = cv2.VideoWriter(out_vid_name, self.encoding, self.fps, self.strategy.dims)
