...
class ObfuscationManager:
    ...
    def obfuscate(self, file_frames_path: str, cover_video_path: str, fourcc: int,
        ...
        def update_progress():
            self.logger.info(f"obfuscation progress:{frame_counter[0]}/{output_frames_amount_with_min_length}%")
    ...
    def untangle(self, obfuscated_video_path: str, fourcc: str,
        ...
        def increase_progress_notify():
            frame_counter[0] += 1
            if progress_tracker is not None:
                self.logger.debug(f"untangle: {frame_counter[0]}/{frame_amount}")
                progress_tracker(frame_counter[0] / frame_amount)
